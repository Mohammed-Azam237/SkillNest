...
export const markLectureAsComplete = async (data, token) => {
  ...
  const toastId = toast.loading("Loading...");
  console.log("📦 Sending this data to markLectureAsComplete:", data);
  try {
    const response = await apiConnector("POST", LECTURE_COMPLETION_API, data, {
      Authorization: `Bearer ${token}`,
    });

    console.log("✅ courseProgressId from backend:", response.data.courseProgressId);
    console.log(
    ...
    if (!response.data.courseProgressId) {
      throw new Error("No courseProgressId returned from backend");
    }

    toast.success("Lecture Completed");
    toast.dismiss(toastId);
    return response.data.courseProgressId;
  } catch (error) {
